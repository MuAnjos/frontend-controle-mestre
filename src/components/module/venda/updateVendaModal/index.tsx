...
import { getVenda, updateVenda } from '@/service/vendaHttp';
...
export default function UpdateVendaModal({
  ...
  const [venda, setVenda] = useState<CreateVendaReq>();
  ...
  useEffect(() => {
    async function fetchProducts() {
      const venda = await getVenda(selectedVenda.id);
      setVenda(venda);
  ...
  async function submit(event: FormEvent) {
    ...
    if (funci !== "") {
      ...
      const response = await updateVenda(nVenda);
      setStatus(response);
    }
  }
